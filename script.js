...
// إصلاح مشكلة شريط تثبيت التطبيق المستمر
let installBannerDismissed = localStorage.getItem('installBannerDismissed');

document.addEventListener('DOMContentLoaded', function() {
    const installBanner = document.getElementById('installBanner');
    const closeInstall = document.getElementById('closeInstall');

    // إذا كان المستخدم قد أغلق الشريط من قبل، لا تعرضه مرة أخرى
    if (installBannerDismissed === 'true' && installBanner) {
    ...
    if (closeInstall) {
        closeInstall.addEventListener('click', () => {
            if (installBanner) {
                ...
                // حفظ في localStorage لمدة 7 أيام
                localStorage.setItem('installBannerDismissed', 'true');
                localStorage.setItem('installBannerDismissedTime', Date.now());
            }
        });
    }

    // تحقق إذا مرت 7 أيام على الإغلاق، أعد عرض الشريط
